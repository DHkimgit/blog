...
Extra: NULL
```
where 절의 조건이 index를 사용하였으며 Extra 가 null 이므로 일반적인 인덱스 스캔을 수행한다는 것을 알 수 있다. 인덱스에 존재하지 않는 컬럼을 읽어오기 위해 테이블에 추가적인 I/O가 수행된다.

인덱스에 비 식별자 열을 포함하는 경우 주의해야 한다. 인덱스의 크기가 커질수록 쿼리의 속도는 느려지고 원하는 결과를 얻기 위해 더 많은 페이지를 탐색해야 한다. 인덱스의 이점을 제대로 누릴 수 없게 된다.
